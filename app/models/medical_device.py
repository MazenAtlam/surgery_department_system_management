...
class MedicalDevices(BaseModel):
    ...
    medical_device_department: so.Mapped[str] = so.mapped_column(sa.ForeignKey('departments.id'),unique=True)
    medical_device: so.Mapped["m.MedicalDevice"] = so.relationship("MedicalDevice", back_populates="medical_device_department")
